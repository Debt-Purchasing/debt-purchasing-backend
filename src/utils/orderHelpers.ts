...
/**
 * Hash OrderTitle fields from order (since we flattened the structure)
 */
export function hashOrderTitle(
...
/**
 * Create struct hash for FullSellOrder (ready for signing)
 */
export function createFullSellOrderStructHash(
  chainId: number,
  contractAddress: string,
  order: IFullSellOrder
): string {
  const titleHash = hashOrderTitle(order);
...
/**
 * Create struct hash for PartialSellOrder (ready for signing)
 */
export function createPartialSellOrderStructHash(
  chainId: number,
  contractAddress: string,
...
/**
 * Verify signature for FullSellOrder (matching contract logic)
 */
export function verifyFullSellOrderSignature(
  chainId: number,
  contractAddress: string,
  order: IFullSellOrder,
...
/**
 * Verify signature for PartialSellOrder (matching contract logic)
 */
export function verifyPartialSellOrderSignature(
  chainId: number,
  contractAddress: string,
  order: IPartialSellOrder,
  expectedSigner: string
...
export async function calculateHealthFactor(
  ...
  try {
    ...
    let totalWeightedCollateralValue = BigInt(0);

    for (const collateral of collaterals) {
      const tokenData = tokenPriceMap.get(collateral.token);
      const liquidationThreshold = liquidationThresholdMap.get(
        collateral.token
      );

      if (!tokenData || !liquidationThreshold) {
        console.warn(`Missing data for collateral token ${collateral.token}`);
        continue;
      }
    ...
    for (const debt of debts) {
      const tokenData = tokenPriceMap.get(debt.token);

      if (!tokenData) {
        console.warn(`Missing price data for debt token ${debt.token}`);
        continue;
      }
    ...
    const healthFactorBig =
      (totalWeightedCollateralValue * BigInt("1000000000000000000")) /
      totalDebtValue;
    return healthFactorBig.toString();
  } catch (error) {
    console.error("Error calculating health factor:", error);
    // Return a safe default value (1.0 in 18 decimals)
